...
bool NetworkInterfaceBSD::bindSocket() {
  ...
  if (bind(m_socket, reinterpret_cast<sockaddr *>(&addr), sizeof(addr)) == -1) {
    Logger::error("Error binding socket to address: ", m_bindAddress, ":", m_port, ". ", strerror(errno));
    return false;
  }
...
bool NetworkInterfaceBSD::sendPacket(const std::vector<uint8_t> &packet, const std::string &address, int port) {
  if (m_socket == -1) {
  ...
  Logger::debug("Sending packet:", "\n  Destination: ", inet_ntoa(broadcastAddr.sin_addr), "\n  Port: ", ntohs(broadcastAddr.sin_port),
                "\n  Packet size: ", packet.size(), " bytes");

  ssize_t bytesSent =
      sendto(m_socket, packet.data(), packet.size(), 0, reinterpret_cast<sockaddr *>(&broadcastAddr), sizeof(broadcastAddr));

  if (bytesSent == -1) {
    Logger::error("NetworkInterfaceBSD: Error sending packet: ", strerror(errno));
    return false;
  }
...
int NetworkInterfaceBSD::receivePacket(std::vector<uint8_t> &buffer) {
  ...
  ssize_t bytesReceived =
      recvfrom(m_socket, m_recvBuffer.data(), m_recvBuffer.size(), 0, reinterpret_cast<sockaddr *>(&senderAddr), &addrLen);
  if (bytesReceived == -1) {
